...
def score(ball_pos,hoop_pos):
    ...
    if len(x)>1:
        ...
        hoop_rebound_zone=10 #define a buffer zone around hoop
...
#Detect if the ball is around the backboard -used to detect shot attempts
...
#Removes inacccurate data points
def clean_ball_pos(ball_pos,frame_count):
    #Remove inaccurate ball size to prevent jumping to wrong ball
    if len(ball_pos)>1:
        #width and Height
        w1=ball_pos[-2][2]
        h1=ball_pos[-2][3]
        ...
        #Ball should not move 4x its diamter within 5 frames
